"""Round-18: is the coupling's benefit spatially structured, and does that change
what a user gets for the observable they actually want?

EXPLORATORY EVIDENCE came from the round-17 PILOT fields (key 1601). This driver
CONFIRMS on a fresh, unused key (1803) and additionally stores BOTH partner
fields, so the two estimators of a nonlinear observable Q can be distinguished:

    Q_of_mean   = Q( (f_A + f_B)/2 )        -- Q applied to the averaged field
    mean_of_Q   = ( Q(f_A) + Q(f_B) )/2     -- Q averaged over replicas

These are different estimators of different quantities. Preserving each replica's
marginal law means mean_of_Q is unbiased for E[Q(single run)]; it says nothing
about Q_of_mean, which targets Q of the mean field.

Predeclared before running: cell N=8192, h=0.005, two-pulse Burgers, 48 fresh
replicates per arm (RAW, FULL). Observables fixed in advance: u at x in
{-2.0, -0.7, +0.1, +1.5}; max|grad u|; the u=0.25 level-crossing location; and
the integrated squared field error (reported as a MEAN, which is the target
metric, not the variance of that scalar).
"""
