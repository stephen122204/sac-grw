"""Round-17: corrected accounting, then one frozen held-out four-arm comparison.

TARGET PROVENANCE. tol = 7e-6 is a PILOT-INFORMED target, chosen after seeing the
round-16 pilot and the budget. The original 5e-6 experiment was never run (budget
gate) and remains recorded that way. This is a separately frozen held-out
experiment, not the original predeclared one.

FROZEN BEFORE ANY EVALUATION DATA ARE TOUCHED: cells N in {2048, 8192}, h=0.005,
two-pulse Burgers; arms SINGLE/RAW/FULL/RQMC; tol=7e-6 expected integrated
squared error on [-5,5]; kappa=1.3; B = ceil(V_hi/(tol/kappa - b2_hi)) with
whole-replicate BOOTSTRAP upper endpoints (approximate at 32 replicates);
b^2 shared across SINGLE/RAW/FULL (marginal law identical) taken from FULL,
RQMC gets its own; pilot key 1601 (replayed), evaluation key 1702 (unused);
32 blocks; B_max=200; projected-cost gate 1200 s AND an elapsed-time stop;
pointwise 95% t per arm, Bonferroni alpha/4 for any joint claim; no topping up.
"""
...
from studies.round17_rqmc import RQMCDiffusion, advance_rank_diffusion
...
OUT = ROOT / 'output/round17_compare_2026_09_10'
...
def main():
    ...
    old = json.load(open(ROOT / 'output/round16_compare_2026_09_10/compare.json'))['stats']
