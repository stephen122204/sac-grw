"""Round-7 part 1: INDEPENDENT verification of Codex's monotonicity counterexample.

CLAIM UNDER TEST (round-6 proposal): for the signed gradient-particle update,
E[U(x,T) | state] is coordinatewise monotone in each particle position with
orientation -sign(m_i). Displacing a POSITIVE-mass particle to the right should
therefore DECREASE the expected terminal field at every probe.

CONFIGURATION (Codex's, reproduced exactly). Production solver in
conditional-mean mode, a=2, u_left=0, m=(+0.5,-0.5), h=0.005, nu=1e-10, two
steps. Negative particle at 0; positive particle at -0.00275 (state L) or
-0.00225 (state R). Both initialise v=(0.5, 0). Probe x=0.00125.

Three independent checks are performed here:
  (1) a hand-traced zero-noise algebra trace, compared against the production
      solver's own output;
  (2) the production solver at sigma=1e-6 over many seeds;
  (3) the margin computation underlying the Gaussian-tail bound, verified
...
OUT = ROOT / 'output/round07_counterexample_2026_09_09'
...
def main():
    ...
    for tag, xp in STATES.items():
        ...
        out = advance_rbgbmc_particles(np.array([xp, 0.0]), M, UL, NU, A_REL, H, K,
    ...
    delta = 1e-4
    print(f"  Codex's displacement cap delta = {delta:.0e}  -> "
