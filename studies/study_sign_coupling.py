"""Round-4: sign-aware coupling of TWO complete gradient-particle runs.

Question: when the two runs develop different signed rank orderings, does a
sign-aware coupling beat ordinary rank reflection in full-field accuracy per
measured work?

Couplings (all pair estimators; every one costs two solver paths):
  raw_rank      Z_B = -Z_A at matched spatial rank      (existing production)
  sign_adjusted Z_B,i = -s_A,i s_B,i Z_A,i              (candidate)
  within_sign   ranks matched WITHIN each sign class, reflected normals
  independent   two independent runs                    (cost/variance diagnostic)

Marginal-law induction. At each diffusion stage the matching and the sign
multipliers are functions of the joint pre-diffusion configuration, hence
measurable before Z is drawn. Given that history, Z is iid N(0,1); multiplying
coordinates by predictable +-1 factors, or applying a predictable permutation,
leaves the vector iid N(0,1). So each run's conditional transition kernel is
unchanged, and by induction over steps each run keeps EXACTLY its original
discrete law, including its discretisation bias. Only the joint law changes.
Signs are never chosen by inspecting the realised noise.

All arms use the inclusive mean-transport schedule, so only the coupling varies.
"""
import argparse
import csv
import hashlib
...
OUT = ROOT / 'output/sign_coupling_2026_09_09'
...
def validate():
    ...
    K = 40
    r = advance_rbgbmc_particles(x, m, ul, .5, 2., .005, K,
