"""Round-9 additions: a transport-aware known-mean control.

Everything in studies/round08_estimators.py is reused unchanged; this module
only adds the frozen-initial-velocity surrogate and its exact mean, so the
round-8 module and its recorded hash stay untouched.

FROZEN-VELOCITY CONTROL. Attach to each particle IDENTITY the velocity its own
initial state gives it,

    v_i^0 = f'( u_right,i^0 ),     u_right,i^0 = u_left + sum_{j <= i} m_j
                                   in the INITIAL sorted order,

and define the surrogate terminal position using that particle's own
identity-tracked Brownian increments from the real nonlinear run:

    X_F,i(T) = x_i^0 + v_i^0 T + sum_k dW_{i,k},
    F(x)     = u_left + sum_i m_i 1{ X_F,i(T) <= x }.

x_i^0, m_i and v_i^0 are DETERMINISTIC functions of the initial particles, and
sum_k dW_{i,k} is exactly N(0, 2 nu T) by the round-8 predictable-permutation
argument, which is unchanged by adding a deterministic shift. Hence

    mu_F(x) = u_left + sum_i m_i Phi( (x - x_i^0 - v_i^0 T) / sqrt(2 nu T) )

is exact for the same discrete initialization. No continuum profile, no PDE
reference, and no trajectory-dependent velocity enters: replacing v_i^0 by a
realized velocity would break this formula and need a separate proof.
"""
import sys
