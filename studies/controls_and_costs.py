...
from studies.instrumented_estimators import (advance_pair_instrumented, pair_field,
                                        heat_pair_field, mu_heat, rb_pair_field)
from studies.transport_control_estimators import (initial_velocity_by_identity,
                                        frozen_pair_field, mu_frozen)
from studies.cubic_flux_transfer import advance_pair_flux
from studies.spectral_reference import solve, evaluate
from studies import twopulse_reference as TP

OUT = ROOT / 'output/controls_and_costs'
R8 = ROOT / 'output/controls_pilot/fields.npz'
...
def main():
    ...
    (OUT / 'costs_control.json').write_text(json.dumps(dict(
        ...
        hashes=dict(r8=hashlib.sha256((ROOT/'studies/instrumented_estimators.py').read_bytes()).hexdigest()[:16],
                    r9=hashlib.sha256((ROOT/'studies/transport_control_estimators.py').read_bytes()).hexdigest()[:16],
