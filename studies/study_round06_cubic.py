"""Round-6 part 4: controlled nonquadratic-flux transfer test.
...
Burgers arm in f alone.

f(u) = u^3/3 gives f'(u) = u^2 >= 0: it vanishes at u = 0 but does not change
sign, so cubic flux does NOT introduce left/right propagation. It introduces a
non-affine characteristic speed with inflection f''(u) = 2u.

PREDECLARED COMPARISONS (fixed before the run; nothing added afterwards).
  Primary   : V_FULL / V_RAW for cubic at h = 0.005 and h = 0.0025.
  Directional: V_FINAL / V_RAW, which Corollary 3 requires to be <= 1.
  Comparator: V_WITHIN / V_RAW, the existing strong practical alternative.
  Control   : the same four ratios for Burgers at identical parameters.
  Diagnostic: one configuration (cubic, h = 0.005) repeated on a second,
              independent seed key.
  Metric    : exact whole-line centred variance of the terminal pair mean, and
              MSE against the independently implemented spectral reference.
  Reading   : a ratio interval containing 1 means the benefit is UNRESOLVED in
              that configuration. It does not establish a Burgers-specific
              effect, and a resolved cubic gain demonstrates transfer to this
              case only, not to arbitrary fluxes or systems.

TIMING. Couplings are interleaved within each seed, the clock covers the solver
only, and no audit-only disagreement scan runs inside the timed region.
"""
import csv
import hashlib
import json
