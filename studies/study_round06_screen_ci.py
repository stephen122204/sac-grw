"""Round-6 part 1c: proper paired intervals for the THREE flagged screen cells.

The round-5 screen documented this reversal rule:

    ratio > 1 with a paired bootstrap interval excluding 1

but the code recorded `verdict = 'REVERSAL' if ratio > 1.0` and computed no
interval at all. Three cells were labelled REVERSAL on a bare point ratio. This
driver reconstructs the missing intervals for exactly those three cells and for
nothing else; the other nine cells stay descriptive point ratios.

Exact bootstrap without recomputing the whole-line integral. For terminal step
fields U_i(x) the union of ALL realizations' breakpoints partitions the line
into cells on which every realization is constant, so with cell lengths l_j

    int Var(U) dx over a subsample S of size R
        = [ sum_{i in S} q_i - (1/R) c^T G c ] / (R - 1),
    q_i = int U_i^2 dx,  G_ik = int U_i U_k dx,  c_i = multiplicity of i in S.

q and G are computed once per cell from the full union grid, so each bootstrap
replicate is an O(R^2) form and is EXACT, not a grid approximation.

Pairing. Raw and sign-adjusted share seed streams, so the same resampled seed
indices are used in both arms of every replicate and the ratio is paired.

Multiplicity. Twelve cells were screened; intervals are reported both at the
nominal 95% level and at a Bonferroni-adjusted 99.6% level (alpha = 0.05/12).
"""
import json
import sys
...
from studies.study_round05_screen import config, variance, NU, T, N, SEEDS

OUT = ROOT / 'output/round06_screen_ci_2026_09_09'
