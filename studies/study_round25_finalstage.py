"""Round-25: archived verification of the final-stage identity (Proposition 2).

The identity is CONDITIONAL on the pre-final paired state s, and it concerns the
WHOLE-LINE integrated variance of the pair mean:

    V_reflect(s) - V_switch(s)
        = (1/4) sum_{i : m_A,i m_B,i < 0} |m_A,i m_B,i| [ g_{2 sigma}(d_i) - |d_i| ],
    g_s(d) = E|d + s Z|,   d_i = a_i - b_i,   sigma = sqrt(2 nu h).

A round-7 check of the residual between this prediction and a measured
difference was reported but never archived. This driver supplies an archived
one, and makes the conditioning explicit: for each of several pre-final states
it (a) evaluates the closed form and (b) estimates the same difference by Monte
Carlo over the final Gaussian stage alone, with the SAME draws used for both
couplings so the difference is paired.

The Monte Carlo estimate integrates over a bounded window wide enough to contain
every realised particle; the identity itself is a whole-line statement and is
...
OUT = ROOT / 'output/round25_finalstage_2026_09_10'
...
def main():
    ...
    fs = json.load(open(ROOT / 'output/round06_finalstep_2026_09_09/finalstep.json'))
