...
\usepackage{etoolbox}
\makeatletter
\patchcmd{\section}{\bf}{\color{sectionblue}\bf}{}{\PackageError{coupling}{Section color patch failed}{Check the arxiv section definition}}
\patchcmd{\subsection}{\bf}{\color{sectionblue}\bf}{}{\PackageError{coupling}{Section color patch failed}{Check the arxiv section definition}}
\patchcmd{\subsubsection}{\bf}{\color{sectionblue}\bf}{}{\PackageError{coupling}{Section color patch failed}{Check the arxiv section definition}}
\makeatother
...
\hypersetup{pdftitle={Sign-Aware Antithetic Coupling for Signed Gradient-Particle Methods},
  ...
  citecolor=citationpink,urlcolor=sectionblue,hypertexnames=false}
...
followed by the discussion and conclusions. Appendices supply proofs, reference
...
Algorithm~\ref{alg:single} and \emph{before} the diffusion. The information available at that stage is denoted by $\Fil_k$ and includes
...
\paragraph{Comparison with the final-stage calculation.}
...
\paragraph{Initially separated signs.}
...
optimal for convex displacement costs \citep{villani2003}. Rank matching \emph{is} that bijection, and within-sign
...
\paragraph{Conditional expectation and control variates.}
...
\paragraph{Profiles and times.} Three fixed initial profiles have different sign structures. We use a
...
result. The region counts use the magnitude threshold specified in
...
\paragraph{Alternating masses.} The interleaved configuration uses alternating masses $\pm1/N$ on a fixed support, giving a reconstructed
...
\paragraph{Profiles.}
The two-pulse profile is \eqref{eq:twopulse}. The other smooth profiles are
\[
 \begin{aligned}
