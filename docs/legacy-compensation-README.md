# Variance-Compensated Relaxation–Brownian Gradient Monte Carlo

Python software and reproducible numerical examples for the paper
*A Variance-Compensated Relaxation–Brownian Gradient Monte Carlo Method for Viscous Burgers'
...
## September 9 compensation revision

The current manuscript leads with variance compensation. Reproduce its seven
full-profile comparisons (50 realizations per arm per cell) and render its three
figures and three tables with:

```bash
python studies/study_compensated_accuracy.py
python studies/render_compensated_revision.py --paper-dir ../paper2-rbgbmc
```

Results are in `output/compensated_accuracy_2026_09_09/`. Completed cells are
reused; remove or move that output directory only if you intend to rerun them.
The Gaussian comparison reads the archived Cole–Hopf reference in
`output/final_prepublication_tests/gbmc_smooth_transient/reference.npz`.
The renderer also reads the 18-cell response summary in
`output/reevaluation_2026_09/reevaluation_summary.json`.

For the additional deterministic response-resolution check:

```bash
python studies/check_response_discretization.py
```

The solver flag is `compensate_transport_variance=True`, using the default
carried-velocity schedule and sampled transport. Do not combine this flag with
`redraw_after_diffusion` or `conditional_mean_transport`; those combinations do
not implement the analyzed method. The residual Brownian variance must be
strictly positive. Conditional variance matching applies between consecutive
interior velocity-sampling stages, not to the first transport-first loop.

The original method remains the solver default for archival reproducibility.
The legacy reproduction targets below describe the earlier uncorrected studies;
they do not verify the new compensation tables or establish convergence of the
corrected method.
...
## Reproduce the Earlier Uncorrected Studies
