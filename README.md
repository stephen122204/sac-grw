...
> each equation. Instead we study whether GRW-based formulations are feasible,
> how closely they reproduce known solutions, and where they fail and why.
> Error metrics are feasibility diagnostics, not accuracy claims.
...
| Burgers  | Cole-Hopf GRW | Exact stationary shock / traveling wave | Approximate. Dominant error is finite-domain BC mismatch (~88% of total) |
...
A standard FD implementation of FHN is kept internally as
`simulate_fitzhugh_nagumo_fd` in `simulation.py`, used only to generate
reference solutions in `verify_solver.py`. It is not the primary solver on this
branch.
...
3. **Reconstruction**: sort globs by position. The cumulative sum of signed weights
   gives u(x, t).
...
(FD_ref - exact) with (GRW - FD_ref) shows that BC-mismatch error is systematic
(~88% of total rms) and GRW shot noise is stochastic (~32%). That confirms the
GRW is correctly implementing the heat equation under its BCs. The dominant
remaining error is the BC mismatch, not a GRW implementation defect.
...
traveling-wave formulation. Globs represent contributions to u_x. The field u
is reconstructed by cumulative summation, exactly as in the heat GRW.
...
Burgers (cole_hopf_grw) -> exact stationary-shock or traveling-wave solution
FHN scalar GRW -> exact traveling-wave solution (multi-time)
```

The Cole-Hopf diagnostics figure (`cole_hopf_diagnostics.png`) shows three
curves per panel: GRW (blue), FD reference heat solve with same BCs (orange),
exact infinite-domain shape (black). The error-decomposition panel quantifies
BC-mismatch vs GRW noise separately.
...
- `comparison_plot.png` - numerical vs reference overlay and pointwise error
...
|   +-- burgers_traveling_wave.json - Cole-Hopf GRW, exact traveling-wave IC
...
| `domain_size`        | number  | Right endpoint L (domain [0, L]) |
